...
![|220](photo/Pasted%20image%2020260107085550.png)
<font color="#ff0000">环路测试的系统框图：</font>
![|300](photo/Pasted%20image%2020260107085655.png)

## 2、SPI总线协议介绍

##### 2.1 技术性能
SPI，全双工三线同步串行外围解耦，主从模式，支持单主多从。Master控制时钟，高位在前，低位在后。两根单向数据线，全双工，实际应用速率可达机Mbps。
![|300](photo/Pasted%20image%2020260107090021.png)
##### 2.2 接口定义：
...
![|340](photo/Pasted%20image%2020260107094510.png)
![|340](photo/Pasted%20image%2020260107094426.png)
![|700](photo/Pasted%20image%2020260107100747.png)

##### 2.4 数据传输
一个 SPI 时钟周期内：
1. 主机通过 MOSI 线发送 1 位数据，从机通过该线读取这 1 位数据 
2. 从机通过 MISO 线发送 1 位数据，主机通过该线读取这 1 位数据
![|260](photo/Pasted%20image%2020260107094725.png)
![|700](photo/Pasted%20image%2020260107100242.png)
主机内的时钟发生器，同时影响主机和从机。
...
![|380](photo/Pasted%20image%2020260107224016.png)
sdk里debug as，run
```c
...
```
![](photo/3b76ed9f3983648687685047c81e8459.jpg)
...
搭建soc
中断设置
![|420](photo/Pasted%20image%2020260107225514.png)
设置 GP Master 接口
![|420](photo/Pasted%20image%2020260107225550.png)
设置复位输出
![|460](photo/Pasted%20image%2020260107225619.png)
设置 PL 时钟
![|460](photo/Pasted%20image%2020260107225806.png)
AXI-SPI IP
![|460](photo/Pasted%20image%2020260107225827.png)
最终形态
![](photo/Pasted%20image%2020260109225213.png)
为了满足三态，顶层应该这样修改
![|207](photo/Pasted%20image%2020260109225313.png)
![|234](photo/Pasted%20image%2020260109225334.png)
![|380](photo/Pasted%20image%2020260109225345.png)
引脚分配对应关系
![](photo/Pasted%20image%2020260109230512.png)
### 3.2 PS部分
下面代码为不断往SPI里写入0x55的程序
运行时遇到问题，打断点定位到这个地方
![|460](photo/Pasted%20image%2020260109230228.png)
大致错误是因为写入了一遍以后读了一边，此时寄存器里已经没有了。
但是下面又对空的寄存器读了一遍，所以导致程序报错
把下面的红框部分注释掉以后，就不在报错了，程序正常运行
最后用过lia抓波形成功写入0x55
![|460](photo/41615746797c0dd5f53d1672ec198410.jpg)
...
2、
![|300](photo/81505ffbe33083b443bc92ea7c93a4e6.png)
