...
![|580](photo/Pasted%20image%2020250602151006.png)
...
![|620](photo/Pasted%20image%2020250602152211.png)
###### 2.配置IIC的EMIO。
使能IIC，使用EMIO将IIC连接到PL端，用于连接HDMI DDC。
![|620](photo/Pasted%20image%2020250602152525.png)
###### 3.配置时钟。
FCLK_CLK0配置为100Mhz，FCLK_CLK1配置为142Mhz，这个时钟用于VDMA读取数据。
由于1080p 60帧的频率为148.5MHz，但包含同步和消隐时间，而VDMA传输的都是有效数据，
因此设置为142MHz就可以满足要求。
![|620](photo/Pasted%20image%2020250602152747.png)
###### 4.配置中断。
使能IRQ_F2P，接收PL端的中断。
![|620](photo/Pasted%20image%2020250602152827.png)
###### 5.配置VDMA
添加VDMA IP
![|420](photo/Pasted%20image%2020250602153312.png)
配置VDMA基本参数
这里主要牵涉到两个接口：
...
![](photo/Pasted%20image%2020250602160408.png)
配置VDMA高级参数
...
![|660](photo/Pasted%20image%2020250602154000.png)
###### 6.添加视频时序控制器VTC
用来产生图像的时序
![|380](photo/Pasted%20image%2020250602154303.png)
配置视频时序控制器参数（简称VTC）
Enable Generation是产生输出时序的使能，选择之后会出现vtiming_out总线信号。
Enable Detetion是用于检测输入时序信号的使能，如果使能，会出现vtiming_in总线，由于本实验为图像输出，因此不使能。
![|700](photo/Pasted%20image%2020250602154430.png)
###### 7.添加AXI流转视频输出控制器
![|300](photo/Pasted%20image%2020250602154636.png)
配置AXI流转视频输出控制器参数
...
详情参考模块用户手册pg044。
![|660](photo/Pasted%20image%2020250602154732.png)
###### 8.添加自定义IP
由于视频有很多分辨率，各种分辨率不相同，需要一个动态时钟控制器，使用黑金例程里的my_ip目录，复制到自己的工程目录下。
![|460](photo/Pasted%20image%2020250602155517.png)
添加ip仓库
![|620](photo/Pasted%20image%2020250602155616.png)
![|660](photo/Pasted%20image%2020250602155710.png)
###### 9.添加动态时钟控制器
这个模块主要功能是根据不同的分辨率配置出不同的时钟输出，本质上是调用了锁相环，
但要注意的是，此模块的==参考时钟必须设置为100MHz==
![|380](photo/Pasted%20image%2020250602155815.png)
###### 10.添加HDMI编码器，用于将RGB数据转换为TMDS信号。
![|300](photo/Pasted%20image%2020250602155930.png)
![|380](photo/Pasted%20image%2020250602160008.png)
###### 11.手动连接时钟信号
![|740](photo/Pasted%20image%2020250602160814.png)
###### 12.手动连接关键信号
![](photo/Pasted%20image%2020250602160857.png)
###### 13.连接中断信号
添加Concat IP，连接中断信号
![|380](photo/Pasted%20image%2020250602161038.png)
![](photo/Pasted%20image%2020250602161146.png)
###### 14.使用vivado自动连线
![](photo/Pasted%20image%2020250602161629.png)
###### 15.导出IIC_0端口
![|540](photo/Pasted%20image%2020250602184153.png)
###### 16.导出编码器端口TMDS和oen
![|500](photo/Pasted%20image%2020250602184139.png)
...
![](photo/Pasted%20image%2020250603005912.png)
连接HDMI输出端口到显示器，编译运行
显示一张图
![](photo/0b1f02776161c4cf7586e2df32c1fff.jpg)
