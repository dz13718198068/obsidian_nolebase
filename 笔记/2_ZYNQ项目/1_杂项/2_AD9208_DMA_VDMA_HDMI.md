...
![](photo/Pasted%20image%2020250603164741.png)

![](photo/Pasted%20image%2020250603104855.png)
## 硬件环境搭建
### 搭建硬件
###### 1.基于VDMA驱动HDMI显示例程
基于例程：
[1_使用VDMA驱动HDMI](1_使用VDMA驱动HDMI.md)
[10_ADDA实验ADDA原理](../../1_正点原子FPGA开发板/1_启明星ZYNQ之FPGA开发/10_ADDA实验ADDA原理.md)
[11_ADDA实验程序设计](../../1_正点原子FPGA开发板/1_启明星ZYNQ之FPGA开发/11_ADDA实验程序设计.md)
...
![](photo/Pasted%20image%2020250603105229.png)
###### 2.添加DMA模块
只打开写通道
![](photo/Pasted%20image%2020250603165145.png)
![](photo/Pasted%20image%2020250603165224.png)
###### 3.添加AXI Interconnect模块
连接S00_AXI和S01_AXI分别到VDMA和DMA的MM2S接口，M00_AXI连接到HP0口。
![](photo/Pasted%20image%2020250603165926.png)
###### 4.增加xlconcat接口
连接到dma的中断口
![](photo/Pasted%20image%2020250603170008.png)
###### 5.添加自定义IP模块ad9280_sample
功能为采集ad9280数据，缓存到FIFO中，并从FIFO中读出转换为AXI4-Stream流数据。
自定义IP在repo文件夹中。
![](photo/Pasted%20image%2020250603170813.png)
添加同步复位模块，并将复位时钟连接到adc_clk，复位输出连接到ad9280的adc_rst_n复位接口。
![](photo/Pasted%20image%2020250603170633.png)
###### 6.添加AXI4-Stream Register Slice模块（可选）
AXI4-Stream Register Slice模块对Stream接口时序会有些提升。
![](photo/Pasted%20image%2020250603170912.png)
![](photo/Pasted%20image%2020250603170939.png)
###### 7.将ADC数据接口以及时钟引出
![](photo/Pasted%20image%2020250603171013.png)
![](photo/Pasted%20image%2020250603171045.png)
保存
...
例化一个信号发送模块
一个coe信号文件存储rom
![](photo/Pasted%20image%2020250603171619.png)
修改一下顶层system_wrapper
![](photo/Pasted%20image%2020250603171457.png)
![](photo/Pasted%20image%2020250603171552.png)
...
![](photo/Pasted%20image%2020250603124501.png)
3.首先要做显示背景，本实验中选择蓝布做背景，利用main.c文件的DemoPrintTest函数，将其他删除，只保留蓝色显示部分。
![](photo/Pasted%20image%2020250604103826.png)
4.在main.c中打开中断控制器，用于DMA的中断。
![](photo/Pasted%20image%2020250604104043.png)
...
![|500](photo/Pasted%20image%2020250604103516.png)
...
![](photo/Pasted%20image%2020250604104408.png)
...
![](photo/Pasted%20image%2020250604104518.png)
判断Sign符号位，赋给不同的指针。
![](photo/Pasted%20image%2020250604104550.png)
由于得到的数据在图像上显示为离散点，为了使波形显示更平滑，进行了描点处理，将数据与前一个数据进行比较，得到差值，并在同一列描点。
![](photo/Pasted%20image%2020250604104731.png)
下面为500KHz未描点的正弦波，都是离散的点：
![](photo/Pasted%20image%2020250604104743.png)
下图为描点之后的效果，平滑了一些。
![](photo/Pasted%20image%2020250604104753.png)
描点函数为draw_point，需要提供横坐标，纵坐标，宽度，高度等参数
![](photo/Pasted%20image%2020250604104839.png)
在adc_dma_ctrl.c的XAxiDma_Adc_Wave函数中，调用frame_copy函数将画布数据copy到图像空间，并刷新Cache，之后打开ADC采集。
![](photo/Pasted%20image%2020250604105131.png)
修改显示分辨率的方法，在display_ctrl.c中修改vMode
![](photo/Pasted%20image%2020250604105256.png)
...
![](photo/Pasted%20image%2020250604105405.png)
在XAxiDma_Adc_Wave函数中也可修改波形的宽度，比如将其改为1024，WAVE_START_COLUMN修改为50，可见效果如下图
![](photo/Pasted%20image%2020250604105817.png)
![](photo/Pasted%20image%2020250604105828.png)
调用draw_wave函数时，Sign符号设置为UNSIGNEDCHAR
![](photo/Pasted%20image%2020250604105931.png)
在adc_dma_ctrl.h文件中，ADC的参数设置如下：
![](photo/Pasted%20image%2020250604110019.png)
...
![](photo/Pasted%20image%2020250603124523.png)
![](photo/Pasted%20image%2020250603124609.png)
###### 2.实现报错
xdc编写错误
![](photo/Pasted%20image%2020250603134114.png)
大概率是xdc问题，修改xdc管教约束就解决了

### 上板验证
![](photo/Pasted%20image%2020250604110150.png)
![](photo/Pasted%20image%2020250603200008.png)
修改背景及画布位置
![](photo/Pasted%20image%2020250603201251.png)
调整画布大小和波形偏移量
![](photo/d7e01a7261f395c98917b95591d2de3.jpg)
增加坐标轴
![](photo/Pasted%20image%2020250604110251.png)
