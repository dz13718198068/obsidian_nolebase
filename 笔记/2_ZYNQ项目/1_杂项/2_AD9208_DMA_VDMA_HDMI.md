...
![](assets/Pasted%20image%2020250603164741.png)

![](assets/Pasted%20image%2020250603104855.png)
## 硬件环境搭建
### 搭建硬件
###### 1.基于VDMA驱动HDMI显示例程
基于例程：
...
![](assets/Pasted%20image%2020250603105229.png)
###### 2.添加DMA模块
只打开写通道
![](assets/Pasted%20image%2020250603165145.png)
![](assets/Pasted%20image%2020250603165224.png)
###### 3.添加AXI Interconnect模块
连接S00_AXI和S01_AXI分别到VDMA和DMA的MM2S接口，M00_AXI连接到HP0口。
![](assets/Pasted%20image%2020250603165926.png)
###### 4.增加xlconcat接口
连接到dma的中断口
![](assets/Pasted%20image%2020250603170008.png)
###### 5.添加自定义IP模块ad9280_sample
功能为采集ad9280数据，缓存到FIFO中，并从FIFO中读出转换为AXI4-Stream流数据。
自定义IP在repo文件夹中。
![](assets/Pasted%20image%2020250603170813.png)
添加同步复位模块，并将复位时钟连接到adc_clk，复位输出连接到ad9280的adc_rst_n复位接口。
![](assets/Pasted%20image%2020250603170633.png)
###### 6.添加AXI4-Stream Register Slice模块（可选）
AXI4-Stream Register Slice模块对Stream接口时序会有些提升。
![](assets/Pasted%20image%2020250603170912.png)
![](assets/Pasted%20image%2020250603170939.png)
###### 7.将ADC数据接口以及时钟引出
![](assets/Pasted%20image%2020250603171013.png)
![](assets/Pasted%20image%2020250603171045.png)
保存
...
例化一个信号发送模块
一个coe信号文件存储rom
![](assets/Pasted%20image%2020250603171619.png)
修改一下顶层system_wrapper
![](assets/Pasted%20image%2020250603171457.png)
![](assets/Pasted%20image%2020250603171552.png)
...
![](assets/Pasted%20image%2020250603124501.png)
3.首先要做显示背景，本实验中选择蓝布做背景，利用main.c文件的DemoPrintTest函数，将其他删除，只保留蓝色显示部分。
![](assets/Pasted%20image%2020250604103826.png)
4.在main.c中打开中断控制器，用于DMA的中断。
![](assets/Pasted%20image%2020250604104043.png)
...
![|500](assets/Pasted%20image%2020250604103516.png)
...
![](assets/Pasted%20image%2020250604104408.png)
...
![](assets/Pasted%20image%2020250604104518.png)
判断Sign符号位，赋给不同的指针。
![](assets/Pasted%20image%2020250604104550.png)
由于得到的数据在图像上显示为离散点，为了使波形显示更平滑，进行了描点处理，将数据与前一个数据进行比较，得到差值，并在同一列描点。
![](assets/Pasted%20image%2020250604104731.png)
下面为500KHz未描点的正弦波，都是离散的点：
![](assets/Pasted%20image%2020250604104743.png)
下图为描点之后的效果，平滑了一些。
![](assets/Pasted%20image%2020250604104753.png)
描点函数为draw_point，需要提供横坐标，纵坐标，宽度，高度等参数
![](assets/Pasted%20image%2020250604104839.png)
在adc_dma_ctrl.c的XAxiDma_Adc_Wave函数中，调用frame_copy函数将画布数据copy到图像空间，并刷新Cache，之后打开ADC采集。
![](assets/Pasted%20image%2020250604105131.png)
修改显示分辨率的方法，在display_ctrl.c中修改vMode
![](assets/Pasted%20image%2020250604105256.png)
...
![](assets/Pasted%20image%2020250604105405.png)
在XAxiDma_Adc_Wave函数中也可修改波形的宽度，比如将其改为1024，WAVE_START_COLUMN修改为50，可见效果如下图
![](assets/Pasted%20image%2020250604105817.png)
![](assets/Pasted%20image%2020250604105828.png)
调用draw_wave函数时，Sign符号设置为UNSIGNEDCHAR
![](assets/Pasted%20image%2020250604105931.png)
在adc_dma_ctrl.h文件中，ADC的参数设置如下：
![](assets/Pasted%20image%2020250604110019.png)
...
![](assets/Pasted%20image%2020250603124523.png)
![](assets/Pasted%20image%2020250603124609.png)
###### 2.实现报错
xdc编写错误
![](assets/Pasted%20image%2020250603134114.png)
大概率是xdc问题，修改xdc管教约束就解决了

### 上板验证
![](assets/Pasted%20image%2020250604110150.png)
![](assets/Pasted%20image%2020250603200008.png)
修改背景及画布位置
![](assets/Pasted%20image%2020250603201251.png)
调整画布大小和波形偏移量
![](assets/d7e01a7261f395c98917b95591d2de3.jpg)
增加坐标轴
![](assets/Pasted%20image%2020250604110251.png)
