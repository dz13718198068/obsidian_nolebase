...
![|620](photo/Pasted%20image%2020250526081050.png)
![|620](photo/Pasted%20image%2020250526081053.png)
# **二、实验设计**
...
![|500](photo/Pasted%20image%2020250526081200.png)
**fifo写模块（代码见文末）**
![|580](photo/Pasted%20image%2020250526081209.png)
![|580](photo/Pasted%20image%2020250526081203.png)
almost_empty打两拍，属于fifo读时钟域，需要打拍来进行同步。
![|390](photo/Pasted%20image%2020250526081215.png)
状态机如图
即将空的时候进入延迟
![](photo/Pasted%20image%2020250526081218.png)
延迟10s以后，等待FIFO IP核内部状态信号更新，进入写操作
![](photo/Pasted%20image%2020250526081221.png)
时钟往FIFO里写入递增数据，almost_full拉高后，进入state0停止写入数据
![](photo/Pasted%20image%2020250526081224.png)
**fifo读模块（完整代码见文末）**
![](photo/Pasted%20image%2020250526081229.png)
fifo_full信号是属于写时钟域的，因此跨时钟收到的信号打两拍进行同步
即将写满，进入延时状态，等待FIFO IP核内部信号稳定
![](photo/Pasted%20image%2020250526081233.png)
延迟结束后进入读状态
![](photo/Pasted%20image%2020250526081237.png)
读状态下fifo_rd_en读使能持续拉高。知道即将读空almost_empty拉高，则关闭读使能fifo_rd_en，进入第一个状态。
**fifo仿真模块（略）**
仿真波形分析
![](photo/Pasted%20image%2020250526081240.png)
仿真波形无误，上板测试
lia信号符合预期![](photo/Pasted%20image%2020250526081243.png)
# **四、代码编写**
## fifo_wr
```verilog
module fifo_wr(
...
source结构
其中ila用于板间信号抓取
![|408](photo/Pasted%20image%2020250526081734.png)