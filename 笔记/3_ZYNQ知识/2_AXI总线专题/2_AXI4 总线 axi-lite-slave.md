...
![|420](assets/Pasted%20image%2020260131143113.png)
![|420](assets/Pasted%20image%2020260131143321.png)
###### 2 选择使用 vivado 自带的 AXI 总线模板创建一个 AXI4-Lite 接口 IP
![|420](assets/Pasted%20image%2020260131143339.png)
###### 3 设置 IP 的名字为 saxi_lite
![|420](assets/Pasted%20image%2020260131143519.png)
模板支持 3 中协议，分别是 AXI4-Full AXI4-Lite AXI4-Stream
###### 4 总线包括 Master 和 Slave 两种模式，这里选择 Slave 模式
![|420](assets/Pasted%20image%2020260131143610.png)
###### 5 选择 Verify Peripheral IP using AXI4 VIP 可以对 AXI4-Lite 快速验证
![|420](assets/Pasted%20image%2020260131143650.png)
...
![|580](assets/Pasted%20image%2020260131144009.png)
###### 7 右击 Generate Output Products生成接口文件
![|380](assets/Pasted%20image%2020260131144423.png)
###### 8 源码+接口文件
![|460](assets/Pasted%20image%2020260131144554.png)


## 二、程序分析
###### 1:axi-lite-slave 的 axi_awready
从机写地址就绪
![|500](assets/Pasted%20image%2020260131145237.png)
###### 2:axi-lite-slave 的 axi_awaddr
核心逻辑是**仅在从机准备接收写地址的时刻，锁存主机的写地址**：
![|500](assets/Pasted%20image%2020260131145710.png)
...
![|500](assets/Pasted%20image%2020260131150752.png)
![|460](assets/Pasted%20image%2020260131150921.png)
...
![|500](assets/Pasted%20image%2020260131151107.png)
![|500](assets/Pasted%20image%2020260131151239.png)
![|500](assets/Pasted%20image%2020260131151255.png)
...
![|500](assets/Pasted%20image%2020260131151650.png)
...
![|500](assets/Pasted%20image%2020260131151841.png)
...
![|500](assets/Pasted%20image%2020260131152011.png)
...
![|500](assets/Pasted%20image%2020260131153740.png)
###### 9:axi-lite-slave 的读数据寄存器
AXI4-Lite 读事务的**核心数据读取与输出逻辑**，分为三部分：
生成读使能信号、
地址解码选择要读取的寄存器、
在合适时机将寄存器数据赋值给读数据总线 `axi_rdata`，最终把数据返回给主机。
![|500](assets/Pasted%20image%2020260131153937.png)
![|500](assets/Pasted%20image%2020260131153952.png)
![|500](assets/Pasted%20image%2020260131154012.png)
可以发现，axi-lite-slave 的代码中没有突发长度的处理，每次只处理一个地址的一个数据。
并且也没有 WLAST 和 RLAST 信号，说明 axi-lite-slave 适合一些低速的数据交互，但是可以节省一些FPGA 的逻辑资源。

## 三、仿真分析
单击仿真
![|380](assets/Pasted%20image%2020260131155904.png)
添加观察信号
![|500](assets/Pasted%20image%2020260131160557.png)
...
写地址有效：
![](assets/Pasted%20image%2020260202095817.png)

写数据有效：
![](assets/Pasted%20image%2020260202095941.png)

写数据，四个reg内写入数据
![](assets/Pasted%20image%2020260202100056.png)

###### 读数据操作

读地址有效：
![](assets/Pasted%20image%2020260202100812.png)

数据有效：
![](assets/Pasted%20image%2020260202101140.png)

数据有效：
![](assets/Pasted%20image%2020260202101341.png)
